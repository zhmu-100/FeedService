...
class PostRepositoryImpl : PostRepository {
    ...
    override fun getPostById(id: String): Post? = transaction {
        ...
        val attachments = PostAttachments.select { PostAttachments.postId eq id }
            .map { row ->
                PostAttachment(
        ...
        val reactions = PostReactions.select { PostReactions.postId eq id }
            .map { row ->
                PostReaction(
        ...
        val comments = PostComments.select { PostComments.postId eq id }
            .map { row ->
                val commentId = row[PostComments.id]
                val commentReactions = CommentReactions.select { CommentReactions.commentId eq commentId }
                    .map { reactionRow ->
                        PostReaction(
    ...
    override fun listUserPosts(userId: String, page: Int, pageSize: Int): Pair<List<Post>, Long> = transaction {
        ...
        val posts = Posts.select { Posts.userId eq userId }
            ...
            .map { row ->
                ...
                val attachments = PostAttachments.select { PostAttachments.postId eq postId }
                    .map { attachmentRow ->
                        PostAttachment(
                ...
                val reactions = PostReactions.select { PostReactions.postId eq postId }
                    .map { reactionRow ->
                        PostReaction(
